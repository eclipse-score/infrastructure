...
def update_index_page(index_path: Path, chapters: list[Section]) -> bool:
    ...
    Returns False (no-op) when the chapter-map markers are absent from
    the page -- this allows the index page to exist without the mindmap.
    """
    original = index_path.read_text(encoding="utf-8")
    if INDEX_MAP_START not in original:
        return False
    pattern = re.compile(
...
def main() -> int:
    ...
    if changed:
        ...
        print(f"No chapter-map update needed for {index_path}")
    return 0
