...
Shared rules are increasingly packaged as separate policy modules (e.g. `score_cpp_policies`, `score_rust_policies`) rather than being folded into toolchain repositories. The architectural rule is that consumers should be able to adopt shared lint, warning, and formatting baselines without changing compiler versions or Bazel toolchain registration. See [chapter 3](03-build-infrastructure.md#33-toolchain-management) for the full toolchain/policy split model. When a policy also exposes selectable runtime features such as sanitizers, [chapter 4](04-testing-infrastructure.md#432-sanitizers-runtime-checks) owns execution; this chapter owns the reusable rule baseline.
