...
That scope also explains why coverage, sanitizers, fuzzing, and profiling belong here. They are runtime techniques that depend on executing software, even when their outputs are later consumed by documentation, compliance, or CI chapters.

**Biggest gap**: testing capability already exists in several strong islands, but shared conventions for framework packaging, traceability, aggregation, dashboards, and advanced runtime analysis are still incomplete across S-CORE.

TODO: where is this discussion about putting tests in a seperate module?

## 4.1 Test Framework Integration 🟡

*Integrating language-specific and target-specific test frameworks with the Bazel build system. Incl Traceability.*

S-CORE already has a workable multi-language testing base. Repositories define Bazel `*_test` targets for C++, Rust, and Python, and higher-level integration environments layer scenario support and target orchestration on top. What is still missing is a clearer shared baseline for how those frameworks are packaged, versioned, and reused so that repositories converge on similar patterns instead of each building a local interpretation.

### 4.1.1 C++ Unit-Test Framework 🟡

*Infrastructure supporting C++ unit testing frameworks.*

C++ testing is one of the most mature paths today. Bazel-integrated frameworks such as GoogleTest already support ordinary unit-style execution well, and existing C++ flows are also ahead on coverage and evidence generation.

**Biggest gap**: The remaining issue is consistency: framework versions and Bazel rule setup still vary per repository.

### 4.1.2 Rust Unit-Test Framework ⚪

*Infrastructure supporting Rust unit testing frameworks.*

Rust tests typically use the native Rust test model through `rules_rust`, which fits well with the Bazel-centered infrastructure direction. The capability exists, but it is less even than the established C++ path, especially around reporting, coverage integration, and traceability metadata.

### 4.1.3 Python Test Frameworks 🟡

*Infrastructure supporting Python testing frameworks.*

Python tests generally rely on pytest-style execution. Either directly (via uv) or through Bazel's Python rules. Note that contrary to C++ and Rust, Pythons pytests are used on all testing levels including for higher-level integration and scenario testing rather than just unit tests.
...
## 4.2 Test Execution & Dynamic Analysis 🟠
