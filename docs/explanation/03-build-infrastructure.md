...
*Core build tooling shared across S-CORE repositories.*
...
**Biggest gap**: 3rd party dependencies are currently pulled directly from modules, not through (our) bazel registry. No shared policy on allowed sources, pinning, version alignment, or license handling exists.
...
### 3.2.3 Tooling and Environment Dependencies 🟡
...
### 3.2.5 Known-Good Integration Sets
...
TODO: where is the integration chapter?
...
A shared Bazel cache should be available to S-CORE CI pipelines.
...
### 3.5.3 Build Performance Visibility 🔴

*Understanding where build time goes and where infrastructure work has the highest payoff.*

Shared build performance visibility should include queue times, cache effectiveness, and critical-path behavior.
